...
export default MapPage = ({ route, navigation }) => {
  ...
  const [lat, changeLat] = useState("lat");
  const [long, changeLong] = useState("long");
  const [state, changeState] = useState("state");
  const [city, changeCity] = useState("city");
  const [zip, changeZip] = useState("zip");
  ...
  function onPress(e) {
    ...
    changeLong(e.nativeEvent.coordinate.longitude);
    ...
    Geocoder.from(
      ...
      .then((json) => {
        ...
        console.log(address);
        changeCity(json.results[0].address_components[2].long_name);
        changeState(json.results[0].address_components[4].long_name);
        changeZip(json.results[0].address_components[6].long_name);
      })
  ...
  function sendPoi() {
    ...
    axios
      .post("https://wherecanibackend-zpqo.onrender.com/poi", poi)
